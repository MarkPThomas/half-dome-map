_polyline.polylineEditor = _polyline.extend({
    ...
    _prepareMapIfNeeded: function() {
        ...
        if(this._options.newPolylines) {
            ...
            that._map.on('click', function(event) {
                ...
                _polyline.PolylineEditor([latLng], that._options, contexts).addTo(that._map);
    ...
    _addMethods: function() {
        ...
        this._parseOptions = function(options) {
            ...
            if(!options.pointIcon)
                this._options.pointIcon = L.icon({ iconUrl: '../public/images/editmarker.png', iconSize: [11, 11], iconAnchor: [6, 6] });
            if(!options.newPointIcon)
                this._options.newPointIcon = L.icon({ iconUrl: '../public/images/editmarker2.png', iconSize: [11, 11], iconAnchor: [6, 6] });
        };
        ...
        this._addMarkers = function(pointNo, latLng, fixNeighbourPositions) {
            var that = this;
            var points = this.getLatLngs();
            var marker = _marker(latLng, {draggable: true, icon: this._options.pointIcon});
            ...
            var newPointMarker = _marker([(latLng.lat + previousPoint.lat) / 2.,
            ...
            newPointMarker.on('contextmenu', function(event) {
                ...
                _polyline.PolylineEditor(points, that._options, contexts, originalPolylineNo + 1)
                                          .addTo(that._map);
        ...
        this._setupDragLines = function(marker, point1, point2) {
            var line1 = null;
            var line2 = null;
            if(point1) line1 = _polyline([marker.getLatLng(), point1], {dasharray: "5,1", weight: 1})
                                .addTo(that._map);
            if(point2) line2 = _polyline([marker.getLatLng(), point1], {dasharray: "5,1", weight: 1})
                                .addTo(that._map);
...
_polyline.polylineEditor.addInitHook(function () {
 ...
 */
_polyline.PolylineEditor = function(latlngs, options, contexts, polylineNo) {
    var result = new _polyline.polylineEditor(latlngs, options);
